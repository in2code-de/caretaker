 ...
 *
 */
class HttpTestService extends TestServiceBase
{
    /**
     * Value Description
     *
    ...
    public function runTest()
    {
        ...
        if (!($expectedStatus && $request_url)) {
            return TestResult::create(Constants::state_undefined, 0, 'LLL:EXT:caretaker/Resources/Private/Language/locallang.xlf:http_no_query');
        }
        ...
        // time-ERROR
        $resultState = Constants::state_ok;
        ...
        elseif ($timeWarning && $time > $timeWarning) {
            if ($resultState < Constants::state_warning) {
        ...
        // Return
        return TestResult::create($resultState, $time, new ResultMessage($message, $values), $submessages);
    }
    ...
    protected function getErrorTypeOnFailure()
    {
        switch ($this->getConfigValue('error_type_on_failure', false, 'sResponse')) {
            case 'WARNING':
                return Constants::state_warning;
                break;
            case 'UNDEFINED':
