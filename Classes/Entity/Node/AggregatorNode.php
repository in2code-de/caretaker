...
abstract class AggregatorNode extends AbstractNode
{
    ...
    public function updateTestResult($options = array())
    {
        ...
        if ($this->getHidden() == true) {
            $groupResult = AggregatorResult::undefined('Node is disabled');
        } else {
            // find children
            ...
            if (count($children) > 0) {
                ...
                $groupResult = AggregatorResult::undefined('No children were found');
            }
            // save to repository if the result differs from the last one
            $resultRepository = AggregatorResultRepository::getInstance();
    ...
    {
        if ($this->getHidden()) {
            $group_result = AggregatorResult::undefined('Node is disabled');
        } else {
            $result_repository = AggregatorResultRepository::getInstance();
     ...
     *
     * @param array <NodeResult> $test_results Child-Results to aggregate
     * @return AggregatorResult Aggregated State
     */
    protected function getAggregatedResult($test_results)
    {
        ...
        if ($num_errors > 0) {
            return AggregatorResult::create(Constants::state_error, $num_undefined, $num_ok, $num_warnings, $num_errors, $message, $submessages);
        } elseif ($num_warnings > 0) {
            return AggregatorResult::create(Constants::state_warning, $num_undefined, $num_ok, $num_warnings, $num_errors, $message, $submessages);
        } elseif ($num_undefined == $num_tests) {
            return AggregatorResult::create(Constants::state_undefined, $num_undefined, $num_ok, $num_warnings, $num_errors, $message, $submessages);
        }
        return AggregatorResult::create(Constants::state_ok, $num_undefined, $num_ok, $num_warnings, $num_errors, $message, $submessages);
    }
    ...
    {
        $aggregator_result_repository = AggregatorResultRepository::getInstance();
