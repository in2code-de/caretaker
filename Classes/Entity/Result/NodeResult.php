...
abstract class NodeResult
{
     ...
     * The submessage
     *
     * @var array array of ResultMessage objects
     */
    ...
    public function __construct(int $uid, int $timestamp, int $state, mixed $message, array $submessages)
    {
        ...
        if (is_a($message, 'ResultMessage')) {
            $this->message = $message;
        } else {
            $this->message = new ResultMessage((string)$message);
        }
     ...
     *
     * @param mixed $message string or ResultMessage
     */
    public function addSubMessage($message)
    {
        if (is_a($message, 'ResultMessage')) {
            ...
            $this->submessages[] = new ResultMessage((string)$message);
        }
    }
     ...
     *
     * @param NodeResult $result
     * @return bool
     */
    public function equals(NodeResult $result)
    {
     ...
     * @return bool
     */
    public function isDifferent(NodeResult $result)
    {
        if ($this->getResultHash() != $result->getResultHash()) {
            return true;
        }
